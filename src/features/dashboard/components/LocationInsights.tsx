...
export function LocationInsights() {
  ...
  const stats = locations.map((loc: Location) => {
    ...
    const totalInput = locLogs.reduce((acc, l) => acc + (parseFloat(l.data.inputQty || '0') || 0), 0);
    const totalProduction = locLogs.reduce((acc, l) => acc + (parseFloat(l.data.productionQty || '0') || 0), 0);
    const totalRejections = locLogs.reduce((acc, l) => acc + (parseFloat(l.data.rejectionQty || '0') || 0), 0);
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <CardTitle className="text-base font-semibold">Multi-Location Insights</CardTitle>
            <CardDescription className="text-xs">Production distribution across sites</CardDescription>
          </div>
          ...
          {stats.slice(0, 5).map((loc) => (
            <div key={loc.id} className="flex items-center justify-between group">
              ...
              <div className="text-right">
                <p className="text-xs font-semibold">{loc.production > 0 ? ((loc.production / (loc.input || 1)) * 100).toFixed(1) : '0'}%</p>
